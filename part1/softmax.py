...
def compute_probabilities(X, theta, temp_parameter):
    ...
    c = np.amax((theta @ X.T) / temp_parameter, axis=0)
    dot_product = (((theta @ X.T) / temp_parameter) - c)
    numerator = np.exp(dot_product)
    denominator = np.sum(np.exp(dot_product), axis=0)
...
def compute_cost_function(X, Y, theta, lambda_factor, temp_parameter):
    ...
    """
    prob_matrix = compute_probabilities(X, theta, temp_parameter).transpose()
    index_j_y = np.arange(Y.shape[0]),Y  #creating two arrays to index in probability matrix that matches label Y
    a = prob_matrix[index_j_y]
    n = Y.shape[0]
    c = -1 / n * np.sum(np.log(a)) + (lambda_factor / 2) * np.sum(theta**2)
    return c
...
def run_gradient_descent_iteration(X, Y, theta, alpha, lambda_factor, temp_parameter):
    ...
    # M = sparse.coo_matrix(([1] * n, (Y, range(n))), shape=(np.shape(prob_matrix))).toarray()
